...
impl<'a> ObservablePlaybackState<'a> {
  ...
  pub fn on_is_playing_changed<F>(&mut self, callback: F) where F: Fn(&bool) -> () + 'static {
    self.is_playing_listeners.push(Box::new(callback));
  }

  pub fn on_current_track_changed<F>(&mut self, callback: F) where F: Fn(&Option<&'a Track>) -> () + 'static {
    self.current_track_listeners.push(Box::new(callback));
  }

  pub fn on_enqueue<F>(&mut self, callback: F) where F: Fn(&(&'a Track, usize)) -> () + 'static {
    self.enqueue_listeners.push(Box::new(callback))
  }

  pub fn on_dequeue<F>(&mut self, callback: F) where F: Fn(&(&'a Track, usize)) -> () + 'static {
  ...
  pub fn dequeue(&mut self, offset: usize) {
    ...
    if i >= self.state.session.len() { return }
